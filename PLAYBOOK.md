...
produces `docs/`; shards set `PLAYWRIGHT_SERVE_ONLY=true` and only
http-server it. The required check is still `playwright-run` (an
aggregator over the shards). The paid
