...
- **The husky `.husky/pre-commit` hook runs `yarn precommit` (eslint + typecheck + jest) on every `git commit`.** Trust it — don't run `yarn precommit` yourself before `git commit` / `git push`. The hook is the gate; running it explicitly too is just doing the same multi-minute pass twice. It enforces the same checks CI's `build` job runs (`eslint src netlify tools --max-warnings 0 && yarn typecheck && yarn test`). If the hook isn't installed (fresh sandbox, `yarn install --ignore-scripts`, etc.), run `yarn install` to wire it in; don't paper over a missing hook by running the gate manually. Running a piece of the gate in isolation while iterating (e.g. one Jest file, `yarn eslint <path>`) is fine — that's a development inner loop, not the commit gate. **Only `git commit` fires the hook.** `githooks(5)` defines `pre-commit` as a `git commit` hook, so *every other* commit-producing command lands ungated: `merge`, `cherry-pick`, `revert`, `rebase`, `am` — all verified on git 2.43, where the hook prints on `git commit` and on none of them. The one you will actually hit is the routine "merge `main` into my branch" before pushing a rework (Share#1769). Nothing local has checked such a commit, so the first real signal is CI, and on a draft PR that means the flip to ready. To force the gate on one, `git commit --amend --no-edit` — that *is* a `git commit`, so `pre-commit` fires.
...
- **PRs: open in draft, land in five steps.** Several PRs are usually
  ...
  **What the gate covers:** `build` (main.yml) and the `test-flows` jobs
  (`playwright-run` — the required aggregator over four shards — and
...
| Writing an assertion that can actually fail — the vacuous-assertion shapes seen in practice, the break-it-and-watch-it-go-red check, and why a feature needs one test of the user's own action (not just proxies), across every way its artifact reaches the viewer | [STYLE.md](STYLE.md) §"Assertions must be able to fail" |
...
| ADF (Align dental) crown meshes: the MetaStream 3 decoder in `src/loader/mts/` — format as decoded (header, arithmetic coder, coordinate trees, the split mesh and its common/rare splits, plug-ins), how it's checked against Viewpoint's DLL (`tools/adf-mts/` oracle + mesh snapshots), what throws `UnsupportedMtsError` | [design/new/adf-mts-decoder.md](design/new/adf-mts-decoder.md) |
...
| GLB export as a Pro feature (`share-140`, #1831): what the OPFS artifact is and how it becomes a download, the code-split pro module served only to subscribers (`pro-module` function + blob import), export tracking ("My Exports"), the per-format feature matrix for OBJ/STL/USD/…, the cross-browser smoke checklist | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) |
| The Bldrs container the OPFS cache stores (`glbContainer.js`): why v3 gzips each chunk as TWO members split at the JSON/BIN boundary, why the codec byte is not the `mode` byte, why v2 artifacts are read in place instead of bumping `schemaVer` (#1855) | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) §1.1a |
| Opening a `.glb.gz` back (#1831): why gzip is decoded as a transport envelope at two seams rather than added as a format, the 512 MiB inflate ceiling, and which paths a `.glb.gz` still cannot take | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) §4.7 |
| Collapsing the batched artifact's per-element meshes into one merged primitive (#1870 reader, #1871 writer behind default-off `glbCollapse`): how `batchedGeometryRanges.js` registers slices of one upload as independent `BatchedMesh` geometry ids, why the shape discriminator is the TABLE rather than the node, why placement is baked while the group's offset stays on the node, the `BLDRS_instance_tables` v2 range canary (exact, rotation-invariant) and the lossy witness Draco exports carry instead, why collapsed artifacts get their own OPFS slot instead of a bump, the portable split, and what the flag flip still needs | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) §1.1d |
| Why the batched artifact's JSON chunk is re-expressed after the write (`glbSlim.js`, #1862) — one bufferView per `(buffer, target, byteStride)` class, glTF-default fields dropped, float32 bounds printed at shortest round-trip, identity `EXT_mesh_gpu_instancing` attributes omitted; and the two that must NEVER be (TRANSLATION, and an empty `attributes`) | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) §1.1c |
...
| Model display controls (`view-140`): auto-color toggle + wireframe, scoped to sub-tree/part/mesh, residency backends for non-batched formats, the `#d:` permalink token (absorbs `hiddenExpressIDs[]` #1250) | [design/new/model-display-controls.md](design/new/model-display-controls.md) |
...
| Measuring a real browser model load — first-mesh/first-pixel timing, the `Preview:` line cross-check against conway's Node harness (conway #544), and the CPU-versus-bandwidth A/B rig (conway #541); how to point it at another model and how to read the JSON record | [design/new/browser-load-measurement.md](design/new/browser-load-measurement.md) |
