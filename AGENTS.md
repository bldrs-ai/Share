...
- **Deps:** this repo uses **yarn Berry (4.9.2)**, vendored at `.yarn/releases/` (`packageManager` + `.yarnrc.yml`). Bootstrapped by `scripts/web-setup.sh`. If `yarn install` fails (`error Error: aborted` — the Claude-Code-on-web egress resets installs), **stop and report**; never hand-edit `yarn.lock`, switch registries, hand-fetch tarballs, or fall back to `npm` (it drifts the tree). In the web sandbox corepack is blocked, so `yarn` is a shim over the vendored binary; if it ever errors reaching `repo.yarnpkg.com`, run `node .yarn/releases/yarn-4.9.2.cjs <cmd>`. Details: [design/new/web-build-environment.md](design/new/web-build-environment.md).
...
| `yarn install` reliability in Claude Code on the web, the cloud setup script, yarn Berry/version | [design/new/web-build-environment.md](design/new/web-build-environment.md) |
