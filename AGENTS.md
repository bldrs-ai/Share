...
| Decoding ADF (Align dental) crown meshes in JS: the MetaStream 3 decoder spec, the arithmetic coder and coordinate trees read from Viewpoint's DLL, the emulator-as-oracle test plan, milestones; `tools/adf-mts/probe.py` for the measurements | [design/new/adf-mts-decoder.md](design/new/adf-mts-decoder.md) |
...
| Collapsing the batched artifact's per-element meshes into one merged primitive (#1831): how `batchedGeometryRanges.js` registers slices of one upload as independent `BatchedMesh` geometry ids, why the shape discriminator is the TABLE rather than the node, why per-element placement is baked while the group's offset stays on the node, and what still has to land (writer, portable re-split, rollout) | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) §1.1d |
