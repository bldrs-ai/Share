...
| Collapsing the batched artifact's per-element meshes into one merged primitive (#1870 reader, #1871 writer behind default-off `glbCollapse`): how `batchedGeometryRanges.js` registers slices of one upload as independent `BatchedMesh` geometry ids, why the shape discriminator is the TABLE rather than the node, why placement is baked while the group's offset stays on the node, the `BLDRS_instance_tables` v2 range canary and why it is exact (Draco exports re-open unpickable), why collapsed artifacts get their own OPFS slot instead of a bump, the portable split, and what the flag flip still needs | [design/new/glb-export-premium.md](design/new/glb-export-premium.md) §1.1d |
