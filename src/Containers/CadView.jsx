	...
	const addThemeListener = () => {
		colorModeContext.addThemeChangeListener((newMode, theme) => {
			if (theme && theme.palette && theme.palette.background && theme.palette.background.paper) {
				const intializedViewer = initViewer(pathPrefix, theme.palette.background.paper, isCameraPerpective);
...
function initViewer(pathPrefix, backgroundColorStr = "#abcdef", isCameraPerpective) {
	...
	function onChangeCameraExam(event) {
		if (isCameraPerpective) {
		}
	...
	v.context.ifcCamera.onChange = onChangeCameraExam;
	v.container = container;
	v.navCube = new NavCube(v);
