 ...
 * @param {string} urlStr URL to process
 * @return {string} URL path without any BLDRS-specific items
...
export function trimToPath(urlStr) {
  ...
  let u
  if (urlStr.indexOf('/') === 0) {
    // If we are passed a relative URL, then assume the base is Github
    u = new URL(urlStr, 'https://github.com')
  } else {
    u = new URL(urlStr)
  }
  ...
  const pathIndex = path.indexOf('share/v/gh')
  if (pathIndex > 0) {
    path = path.substring(pathIndex + 'share/v/gh'.length)
  }

  const firstSlashNdx = path.indexOf('/')
  if (firstSlashNdx === -1) {
    throw new Error('Expected at least one slash for file path: ' + urlStr)
  }

  return path.substring(firstSlashNdx)
}
