...
export default function IssuesControl({viewer}) {
  ...
  useEffect(() => {
    (async () => {
      await onLoad(location, setText, setNext)
    })()
  }, [location])
 ...
 */
async function onLoad(location, setText, setNext) {
  debug().log('IssuesControl#onLoad')
  onHash(location, setText, setNext)
  window.onhashchange = () => onHash(location, setText, setNext)
}
 ...
 * @param {function} setNext React state setter for next Link
 */
export async function onHash(location, setText, setNext) {
